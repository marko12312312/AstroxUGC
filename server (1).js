...
// TRANSACTIONS (cookie via legacy economy API)
app.get('/api/transactions', async (req, res) => {
  try {
    const cursor = req.query.cursor ? `&cursor=${req.query.cursor}` : '';
    const response = await fetch(
      `https://economy.roblox.com/v2/groups/${GROUP_ID}/transactions?transactionType=Sale&limit=50${cursor}`,
...
app.listen(PORT, () => {
  ...
  console.log(`   Cookie: ${ROBLOSECURITY ? 'SET ✓' : 'NOT SET ✗'}\n`);
});
